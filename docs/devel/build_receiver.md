...
OpenTelemetry 将 [分布式追踪 (distributed tracing)](https://opentelemetry.io/docs/concepts/glossary/#distributed-tracing) 定义为：
...
> [!NOTE]
>
> 在这里，为了简单起见，我们在 `otlp` 导出器配置中使用了 `insecure` 标志；在生产环境中运行 Collector 时，您应该遵循此 [指南](https://opentelemetry.io/docs/collector/configuration/#setting-up-certificates) 使用 TLS 证书进行安全通信，或使用 mTLS 进行双向认证。
...
在 OTel 的世界中，所有遥测数据都是由 `Resource` 生成的。以下是根据 [OTel 规范](https://opentelemetry.io/docs/specs/otel/resource/sdk)的定义：
...
这些指南被称为 [资源语义约定 (Resource Semantic Conventions)](https://opentelemetry.io/docs/specs/semconv/resource/)，并记录在 OTel 规范中。
...
资源语义约定还指定了规定性的属性名称和常用值，以表示在不同领域（如 [计算单元 (compute unit)](https://opentelemetry.io/docs/specs/semconv/resource/#compute-unit)、[环境 (environment)](https://opentelemetry.io/docs/specs/semconv/resource/#compute-unit) 等）中常见且适用的遥测数据产生实体。

对于 `BackendSystem` 实体，它具有代表与 [操作系统 (Operating System)](https://opentelemetry.io/docs/specs/semconv/resource/os/) 和 [云 (Cloud)](https://opentelemetry.io/docs/specs/semconv/resource/cloud/) 相关信息的字段。我们将使用资源语义约定指定的属性名称和值来在它的 `Resource` 上代表该信息。
...
> - 将 span 的 `Kind` 设置为 `ptrace.SpanKindServer`。请参阅 trace 规范中的 [SpanKind 部分](https://opentelemetry.io/docs/specs/otel/trace/api/#spankind)以了解如何正确定义 SpanKind。
